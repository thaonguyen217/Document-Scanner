...
def getContours(img):
    ...
    for cnt in contours:
        ...
        if area > 6000:
            cv2.drawContours(imgContours, cnt, -1, (255, 0, 255), 2)  # contour index = -1: draw all the contours
        ...
        cv2.drawContours(imgContours, biggest, -1, (255, 255, 0), 5) # contour index = -1: draw all the contours
    return biggest
...
boxes = pytesseract.image_to_boxes(imgText, lang=lang)
for box in boxes.splitlines():
    print(box)
    box = box.split(' ')
    x, y, w, h = int(box[1]), int(box[2]), int(box[3]), int(box[4])
    cv2.rectangle(imgResult, (x,hImg-y), (w,hImg-h), color=(255,255,0), thickness=1 )
